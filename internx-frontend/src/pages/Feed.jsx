...
/* ── Post Card ─────────────────────────────────────────────────────────── */
function PostCard({ post, onLike, onComment, onDelete, isOwn }) {
    ...
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, marginBottom: 16, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '16px 16px 12px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {post.authorHeadline && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{post.authorHeadline}</div>}
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{timeAgo(post.createdAt)}</div>
        </div>
        {isOwn && (
          <button onClick={() => onDelete(post.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 4, borderRadius: 6, fontSize: 18, lineHeight: 1 }} title="Delete post">⋯</button>
        )}
      </div>
      ...
      {(likeCount > 0 || commentCount > 0) && (
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 16px', fontSize: 13, color: 'var(--text-muted)', borderTop: '1px solid var(--border)' }}>
      ...
      <div style={{ display: 'flex', borderTop: '1px solid var(--border)', padding: '2px 8px' }}>
        ...
        <div ref={reactionRef} style={{ position: 'relative', flex: 1 }}>
          <button
            ...
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 0', background: 'none', border: 'none', cursor: 'pointer', borderRadius: 8, color: isLiked ? 'var(--blue)' : 'var(--text-muted)', fontWeight: isLiked ? 600 : 400, fontSize: 14, transition: 'background .15s' }}
          ...
          {/* Reaction picker */}
          {showReactions && (
            <div onMouseLeave={() => setShowReactions(false)} style={{ position: 'absolute', bottom: '110%', left: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 24, padding: '6px 10px', display: 'flex', gap: 4, boxShadow: '0 4px 20px rgba(0,0,0,.12)', zIndex: 10, whiteSpace: 'nowrap' }}>
              {REACTIONS.map(r => (
                <button key={r.label} title={r.label} onClick={() => { onLike(post.id, isLiked); setShowReactions(false) }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 22, padding: '2px 4px', borderRadius: 8, transition: 'transform .1s' }}
        ...
        <button
          ...
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 0', background: 'none', border: 'none', cursor: 'pointer', borderRadius: 8, color: 'var(--text-muted)', fontSize: 14, transition: 'background .15s' }}
          onMouseOver={e => e.currentTarget.style.background = 'var(--bg)'}
          onMouseOut={e => e.currentTarget.style.background = 'none'}
        >
              ...
              {comments.map(c => {
                ...
                return (
                  <div key={c.id} style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                    <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--blue)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0, overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3, marginLeft: 4 }}>{timeAgo(c.createdAt)}</div>
                    </div>
                  </div>
                )
              })}
...
export default function Feed() {
  ...
  const handleLike = async (postId, isLiked) => {
    try {
      ...
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, isLikedByCurrentUser: !isLiked, likeCount: (p.likeCount || 0) + (isLiked ? -1 : 1) } : p))
    } catch { toast('Failed to update like', 'danger') }
  }
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto' }}>
      ...
      {loading ? (
        ...
        <>
          {posts.map(post => (
            <PostCard key={post.id} post={post} onLike={handleLike} onComment={handleComment} onDelete={handleDelete} isOwn={post.userId === user?.id} />
          ))}
